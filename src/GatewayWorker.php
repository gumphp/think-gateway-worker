<?php
namespace gumphp\gatewayworker;

use Exception;
use GatewayWorker\BusinessWorker;
use GatewayWorker\Gateway;
use GatewayWorker\Register;
use gumphp\gatewayworker\handler\Events;
...
class GatewayWorker
{
    ...
    public function startBusinessWorker()
    {
        ...
        $worker->eventHandler    = $this->config('event_handler', Events::class);
    }
    ...
    {
        $register = new Register($this->config('register')); # 允许注册通讯的地址
        $register->name = $this->config('register_name');
    }

    public function ready($appName)
    {
        ...
        $path = runtime_path('gateway-worker');
        if (!is_dir($path)) mkdir($path);
        $unique_prefix = "gateway-worker-" . \str_replace('\\', '_', $appName);
    ...
    public static function startAll($appName)
    {
        (new static())->ready($appName);
        Worker::runAll();
    }
    ...
    protected function getConfigs()
    {
        ...
        $this->configs = config("gatewayworker.providers.{$this->appName}");
        if (empty($this->configs)) {
            throw new Exception("{$this->appName}'s GatewayWorker config doesn't exist");
        }
        return $this->configs;
    }
}